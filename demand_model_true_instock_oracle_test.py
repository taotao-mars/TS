...
# ============================================================
# ORACLE TEST: use TRUE future in_stock as external feature
# ============================================================

def build_true_instock_hat_from_data(
    data_raw1,
    target_col_candidates=("in_stock_dph", "true_instock_dph", "future_instock_dph"),
):
    """
    Build an oracle exposure_hat using TRUE in_stock from data_raw1.

    This is NOT a deployable setting.
    It is only an upper-bound diagnostic:
        If true future in_stock helps demand model a lot,
        then improving in_stock_hat is valuable.
        If even true future in_stock does not help,
        then in_stock is probably not the bottleneck.
    ...
    Here pred_instock_dph is intentionally set to true in_stock.
    The downstream loader will convert it to:
        external_instock_dph_hat_log = log1p(pred_instock_dph)
    """
    df = data_raw1.copy()

    if "asin" not in df.columns or "order_week" not in df.columns:
        raise ValueError("data_raw1 must contain asin and order_week.")
    ...
    if true_col is None:
        raise ValueError(
            "Cannot find true in_stock column in data_raw1. "
            f"Tried: {target_col_candidates}. "
            "Please check your column name."
        )

    out = df[["asin", "order_week", true_col]].copy()
    out = out.rename(columns={true_col: "pred_instock_dph"})
    ...
    # Deduplicate ASIN/week if needed.
    out = (
        out.groupby(["asin", "order_week"], as_index=False)
        .agg(pred_instock_dph=("pred_instock_dph", "mean"))
    )
    ...
    print("\n" + "=" * 100)
    print("TRUE / ORACLE IN_STOCK HAT BUILT")
    print("=" * 100)
    print(f"Using true column from data_raw1: {true_col}")
    print(out[["pred_instock_dph"]].describe().round(4).to_string())
    print("\nThis is an oracle diagnostic, not a deployable feature.")

    return out


def run_demand_with_true_instock_hat(
    data_raw1,
    scot_df,
    n_asins=5000,
    seed=42,
    ...
    """
    Oracle demand-model test.

    Everything else stays the same as the in_stock-only demand experiment,
    except the external in_stock feature uses TRUE future in_stock instead
    of the predicted anchor_attention in_stock_hat.
    ...
    It does NOT use:
        ...
        external_buy_box_dph_hat_log
    """
    true_instock_hat = build_true_instock_hat_from_data(data_raw1)

    result = run_nb_all_sample_scot_intersection_with_external_exposure(
        data_raw1=data_raw1,
        scot_df=scot_df,
        exposure_hat_df=true_instock_hat,
    ...
    print("\n" + "=" * 100)
    print("ORACLE TRUE IN_STOCK CONNECTED TO DEMAND MODEL")
    print("=" * 100)
    print("Used feature:")
    print("  external_instock_dph_hat_log = log1p(TRUE in_stock_dph)")
    print("\nNot used:")
    print("  external_total_dph_hat_log")
    print("  external_buy_box_dph_hat_log")
    print("\nInternal exposure decoder should be disabled:")
...
# ============================================================
# Single usage: oracle true in_stock test
# ============================================================
#
# result_demand_true_instock = run_demand_with_true_instock_hat(
#     data_raw1=data_raw1,
#     scot_df=scot_df,
#
#     n_asins=5000,
