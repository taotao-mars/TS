...
def run_nb_high_sparse_from_sample_scot_intersection(
    ...
    """
    print("=" * 80)
    print("NB HIGH-SPARSE | SAMPLE 5000 THEN KEEP SCOT INTERSECTION")
    print("=" * 80)
...
# =====================================================
# 12. Train on sample-SCOT intersection without sparse filtering
# =====================================================
...
def run_nb_all_sample_scot_intersection(
    ...
    """
    Sample 5000 ASINs, keep the SCOT intersection, train on all intersection ASINs,
    then align with SCOT and compute WAPE.
    """
    print("=" * 80)
    print("NB ALL-ASIN | SAMPLE 5000 THEN KEEP SCOT INTERSECTION")
    print("=" * 80)

    data_train_raw, sample_asin_df, intersect_asin_df = (
        prepare_data_from_sample_scot_intersection(
            data_raw1=data_raw1,
            scot_df=scot_df,
            n_asins=n_asins,
    ...
    print("=" * 80)
    print("Training ASINs:", data_train_raw["asin"].nunique())
    print("Training rows:", len(data_train_raw))

    # Diagnostic grouping only. No filtering by sparse group is applied.
    data_grouped, asin_stats = add_zero_rate_group(
        data_train_raw,
        zero_thresholds=(0.4, 0.7),
    )

    print("\nSparse-group counts for diagnostics only:")
    print(
        data_grouped
    ...
    result = {
        ...
        "data_grouped_for_diagnostics": data_grouped,
        "asin_stats": asin_stats,
        "sample_asin_df": sample_asin_df,
        "intersect_asin_df": intersect_asin_df,
        "removed_extreme": removed_extreme,
    ...
    if run_wape:
        real_scot_outputs = run_high_sparse_scot_alignment_wape(
            result=result,
            scot_df=scot_df,
            data_raw1=data_raw1,
            remove_oos_dp=remove_oos_dp,
        ...
        result["real_scot_outputs"] = real_scot_outputs
...
# Example for all intersection ASINs, no sparse filtering:
#
...
# )
#
# forecast_df_all = result_all_intersection["forecast_df"]
# scot_outputs_all = result_all_intersection["real_scot_outputs"]
# wape_df_all = scot_outputs_all["wape_df"]
# p50_wape_all = scot_outputs_all["p50_wape"]
# p70_wape_all = scot_outputs_all["p70_wape"]
