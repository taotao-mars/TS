...
Only change compared with your NB version:
...
def load_real_data(data_raw):
    ...
    # Lag-safe DPH.
    # Use only historical traffic information.
    df["in_stock_dph"] = df.groupby("ASIN")["in_stock_dph"].shift(1).fillna(0)

    # Normalize holiday indicator features.
    ...
    # previous week + holiday week
    # NOTE: This follows your original NB version.
    # -------------------------------------------------
    if len(holiday_cols) > 0:
        ...
        for c in holiday_cols:
            ...
            holiday_window = np.maximum(
                holiday_window,
                np.maximum(cur, prev_window)
            )
    ...
    for asin, group in df.groupby("ASIN"):
        ...
        # Enhanced temporal features for history encoder.
        features = np.stack([
            v_t,
            b_t,
            d_t,
        ...
        # Conditional z context: our_price + in_stock_dph + holiday indicator features
...
def main(
    ...
    print("TCN + SparsePeakAttn + ENN | Tail-aware NB + Promo/Quarterly Encoder + Lag-DPH Z Context | No Two-Head")
    print("=" * 60)
...
def run_nb_high_sparse_experiment(
    ...
    return model, forecast_df, tr_ld, va_ld, data_high_sparse_nb, asin_zero_stats
...
#
# model, forecast_df, tr_ld, va_ld, data_high_sparse_nb, asin_zero_stats = (
