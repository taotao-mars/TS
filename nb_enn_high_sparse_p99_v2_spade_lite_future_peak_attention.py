...
Sparse peak attention is future-conditioned to reduce peak carry-over
...
class SparsePeakAttention(nn.Module):
    ...
    def __init__(
        ...
        self.peak_gate_floor = peak_gate_floor
        self.peak_gate_power = peak_gate_power
...
class TCN_ENN(nn.Module):
    def __init__(
        ...
        peak_gate_floor=0.15,
        peak_gate_power=1.0,
        future_peak_start_idx=2
    ):
        super().__init__()

        self.d_z = d_z
        self.horizon = horizon
        self.future_peak_start_idx = future_peak_start_idx
...
def main(
    ...
    print(f"beta_peak: {beta_peak} | peak_gate_floor: {peak_gate_floor} | peak_gate_power: {peak_gate_power}")
...
def nb_peak_gate_diagnostic(model, va_ld):
    ...
    with torch.no_grad():
        for b in va_ld:
            s = model._future_peak_strength(b["future_context"])
            if s is not None:
                vals.append(s.detach().cpu().numpy())

    if len(vals) == 0:
        print("No future peak strength available.")
        return pd.DataFrame()
    ...
    out = pd.DataFrame([{
        "n_windows": len(arr),
        "mean_future_peak_strength": float(arr.mean()),
        "p50_future_peak_strength": float(np.quantile(arr, 0.5)),
        "p90_future_peak_strength": float(np.quantile(arr, 0.9)),
        "share_any_future_peak": float((arr > 0).mean()),
    }])

    print("\n" + "=" * 80)
    print("FUTURE PEAK GATE DIAGNOSTIC")
    print("=" * 80)
    print(out.T)
...
def run_wape_and_underbias(model, forecast_df, va_ld, M_diag=100, threshold=0.5):
    ...
    try:
        outputs["peak_gate_diag"] = nb_peak_gate_diagnostic(model, va_ld)
    except Exception as e:
        print("Peak gate diagnostic skipped:", repr(e))
