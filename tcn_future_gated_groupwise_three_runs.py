"""
Group-wise three-run experiment for:
...
3. Split ASINs into three groups:
      low_sparse  : zero_rate < 0.4
      mid_sparse  : 0.4 <= zero_rate < 0.7
      high_sparse : zero_rate >= 0.7
4. Train one separate model per group.
5. Evaluate WAPE / penalty using your existing business functions:
      calculate_wape_using_lp_oos2
      quick_error_check
6. Also report diagnosis metrics for each group.
...
class TCN_ENN(nn.Module):
    def __init__(self, input_dim=11, context_dim=2, d_model=32, d_z=16, horizon=20, prior_scale=0.3):
        super().__init__()
        self.d_z = d_z
        self.horizon = horizon

        self.encoder = TCNSparseAttnEncoder(input_dim=input_dim, d_model=d_model, horizon=horizon)
...
# =====================================================
# 7. Run all three groups
# =====================================================

def run_three_group_experiment(
    data_raw1,
    n_asins=5000,
    ...
    group_order = ['low_sparse', 'mid_sparse', 'high_sparse']
    results = {}

    for group_name in group_order:
        data_g = data_grouped[data_grouped['zero_group'] == group_name].copy()
        if data_g['asin'].nunique() == 0:
            print(f"Skipping {group_name}: no ASINs")
            continue

        results[group_name] = run_one_group(
            data_g,
    ...
    summary_df = pd.DataFrame([results[g]['summary'] for g in results])

    all_forecast_df = pd.concat(
        [results[g]['forecast_df'] for g in results],
        ignore_index=True
    ) if len(results) > 0 else pd.DataFrame()

    print("\n" + "=" * 80)
    print("THREE-GROUP EXPERIMENT SUMMARY")
    print("=" * 80)
    print(summary_df)

    return results, summary_df, all_forecast_df, asin_stats
...
results_by_group, group_run_summary_df, all_group_forecast_df, asin_zero_stats = run_three_group_experiment(
    data_raw1,
    n_asins=5000,
    zero_thresholds=(0.4, 0.7),
    prior_scale=0.3,
