...
NB-v2-trend changes compared with your NB-v1 version:
...
- add 4 dynamic historical trend features:
  active_rate_trend, available_dph_trend, positive_mean_trend, log_demand_slope_13
"""
...
def _rolling_slope(arr, window):
    """
    Rolling linear slope on a numeric series.

    Used on log1p(demand), so it is stable for sparse demand with many zeros.
    The slope at time i only uses observations up to i, so it is history-safe.
    """
    ...
    for i in range(len(arr)):
        ...
        denom = np.sum(x ** 2)
        out[i] = float(np.sum(x * y_centered) / denom) if denom > 0 else 0.0
...
def load_real_data(data_raw):
    ...
    History features:
      ...
      25 active_rate_trend
      26 available_dph_trend
      27 positive_mean_trend
      28 log_demand_slope_13
    """
    ...
    for asin, group in df.groupby("ASIN"):
        ...
        # -------------------------------------------------
        # Dynamic historical trend features.
        # These are computed only from history up to each time t.
        # They are not future context and not static features.
        # -------------------------------------------------

        # Occurrence trend:
        # positive means the ASIN has become more frequently active recently.
        active_rate_trend = active_rate_4 - active_rate_13

        # Available exposure / traffic trend:
        # in_stock_dph is interpreted as in-stock page-click exposure.
        # positive means recent available exposure is improving.
        available_dph_trend = (
        ...
        # Positive-demand magnitude trend:
        # positive means recent nonzero demand size is above its 13-week level.
        positive_mean_trend = (
        ...
        # Stable demand-level slope on log1p(demand).
        log_demand_slope_13 = _rolling_slope(np.log1p(demand), 13)
...
class TCNSparseAttnEncoder(nn.Module):
    def __init__(self, input_dim=29, d_model=32, horizon=20):
        super().__init__()
...
def main(
    ...
    print("NB-v2-trend | Enhanced 29-feature TCN + SparsePeakAttn + ENN | trend features + tail-aware NB + p99 clean high-sparse")
    print("=" * 60)
