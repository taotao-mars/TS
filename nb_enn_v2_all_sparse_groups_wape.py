...
def train(
    ...
    for epoch in range(epochs):
        ...
        for b in tr_ld:
            ...
            preds = model(x, future_context, nZ=nZ)
...
def main(
    ...
    print("NB-v2 | Enhanced 25-feature TCN + SparsePeakAttn + ENN | Tail-aware NB + p99 clean high-sparse")
    print("=" * 60)
...
def run_wape_and_underbias(model, forecast_df, va_ld, M_diag=100, threshold=0.5):
    ...
    NB-v2 also prints true mean vs predicted mean / active magnitude gap.
    """
    outputs = {}

    print("\n" + "=" * 80)
    print("NB-v2 FORECAST DISTRIBUTION CHECK")
    print("=" * 80)
    nb_dist_check = nb_forecast_distribution_check(forecast_df)
    print(nb_dist_check.T)

    print("\n" + "=" * 80)
    print("NB-v2 MAGNITUDE GAP - ACTIVE WEEKS ONLY")
    print("=" * 80)
...
# =====================================================
# 14. Run NB-v2 on All Sparse Groups
# =====================================================

def run_nb_one_sparse_group_experiment(
    data_grouped,
    group_name,
    prior_scale=0.3,
    epochs=60,
    ...
    """
    Train and evaluate NB-v2 on one zero-rate group:
      low_sparse / mid_sparse / high_sparse

    This keeps the NB-v2 model unchanged.
    """
    data_group = data_grouped[
        data_grouped["zero_group"] == group_name
    ].copy()
    ...
    if remove_extreme_asins:
        data_group, removed_extreme_asin_stats, extreme_cap = filter_extreme_asins_by_positive_p99_for_nb(
            data_group,
            demand_col="fbi_demand",
            asin_col="asin",
            q=extreme_q
    ...
    print("\n" + "=" * 100)
    print(f"NB-v2 ON GROUP: {group_name}")
    print("=" * 100)
    print(f"{group_name} ASINs:", data_group["asin"].nunique())
    print(f"{group_name} rows :", len(data_group))
    print(
        f"{group_name} zero rate:",
        (
            pd.to_numeric(
                data_group["fbi_demand"],
                errors="coerce"
            ).fillna(0) == 0
        ).mean()
    )
    ...
    if data_group["asin"].nunique() == 0 or len(data_group) == 0:
    ...
    summary = summarize_nb_group_result(
...
def _safe_get_wape_value(wape_obj, key):
    """
    Extract scalar value from quick_error_check output.
    """
    try:
        if isinstance(wape_obj, pd.Series):
            return float(wape_obj[key])
        if isinstance(wape_obj, pd.DataFrame):
            if key in wape_obj.index:
                return float(wape_obj.loc[key].values[0])
            if key in wape_obj.columns:
                return float(wape_obj[key].values[0])
        if isinstance(wape_obj, dict):
            return float(wape_obj[key])
    except Exception:
        pass
...
def summarize_nb_group_result(group_name, forecast_df, nb_outputs, filter_info):
    """
    Create a compact one-row summary for one sparse group.
    """
    ...
    p50_wape = nb_outputs.get("p50_wape", None) if nb_outputs is not None else None
    p70_wape = nb_outputs.get("p70_wape", None) if nb_outputs is not None else None

    nb_dist_check = nb_outputs.get("nb_dist_check", pd.DataFrame()) if nb_outputs is not None else pd.DataFrame()
    nb_mag_gap = nb_outputs.get("nb_mag_gap", pd.DataFrame()) if nb_outputs is not None else pd.DataFrame()

    row = {
        "group": group_name,
        "status": "ok",
        "n_rows_forecast": len(forecast_df),
        "n_asins_forecast": forecast_df["asin"].nunique(),
        "true_mean": forecast_df["fbi_demand"].mean(),
        "p50_mean": forecast_df["p50_amxl"].mean(),
        "p70_mean": forecast_df["p70_amxl"].mean(),
        "true_zero_rate": (forecast_df["fbi_demand"] == 0).mean(),
        "true_active_ratio": (forecast_df["fbi_demand"] > 0).mean(),
        "p50_active_ratio": (forecast_df["p50_amxl"] > 0.5).mean(),
        "p70_active_ratio": (forecast_df["p70_amxl"] > 0.5).mean(),
        "n_removed_extreme_asins": filter_info.get("n_removed_extreme_asins", np.nan),
        "extreme_cap": filter_info.get("extreme_cap", np.nan),
        "p50_amxl_penalty": _safe_get_wape_value(p50_wape, "p50_amxl_penalty"),
        "p50_amxl_overbias": _safe_get_wape_value(p50_wape, "p50_amxl_overbias"),
        "p50_amxl_underbias": _safe_get_wape_value(p50_wape, "p50_amxl_underbias"),
        "p70_amxl_penalty": _safe_get_wape_value(p70_wape, "p70_amxl_penalty"),
        "p70_amxl_overbias": _safe_get_wape_value(p70_wape, "p70_amxl_overbias"),
        "p70_amxl_underbias": _safe_get_wape_value(p70_wape, "p70_amxl_underbias"),
        "p50_penalty_diff": nb_outputs.get("p50_penalty_diff", np.nan) if nb_outputs is not None else np.nan,
        "p70_penalty_diff": nb_outputs.get("p70_penalty_diff", np.nan) if nb_outputs is not None else np.nan,
    }
    ...
    if len(nb_mag_gap) > 0:
        for col in [
            ...
            if col in nb_mag_gap.columns:
                row[col] = nb_mag_gap.iloc[0][col]

    return pd.DataFrame([row])


def run_nb_all_sparse_groups_experiment(
    data_raw1,
    n_asins=5000,
    zero_thresholds=(0.4, 0.7),
    groups=("low_sparse", "mid_sparse", "high_sparse"),
    prior_scale=0.3,
    epochs=30,
    history=52,
    horizon=20,
    ...
    Simple comparison experiment:
      1. sample ASINs once
      2. split into low/mid/high sparse groups
      3. train one NB-v2 model per group
      4. compute WAPE / diagnostics per group

    For quick testing, use epochs=20 or 30.
    For final numbers, use epochs=60.
    """
    print("\n" + "=" * 100)
    print("NB-v2 ALL SPARSE GROUPS EXPERIMENT")
    print("=" * 100)
    ...
    for group_name in groups:
        result = run_nb_one_sparse_group_experiment(
    ...
    print("\n" + "=" * 100)
    print("NB-v2 ALL GROUPS SUMMARY")
    print("=" * 100)
    ...
    return results, summary_df, asin_zero_stats
...
# =====================================================
# 15. Execute All Sparse Groups
# =====================================================
# Quick test:
#
# all_group_results, all_group_summary_df, asin_zero_stats = (
#     run_nb_all_sparse_groups_experiment(
#         data_raw1,
#         n_asins=5000,
#         zero_thresholds=(0.4, 0.7),
#         groups=("low_sparse", "mid_sparse", "high_sparse"),
...
#
# Final run:
# Change epochs=60 and optionally M_eval=100.
