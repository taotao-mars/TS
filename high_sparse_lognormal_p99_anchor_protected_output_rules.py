...
def predict_hybrid_quantile(
    ...
    """
    Hybrid business-output forecast.

    Motivation:
    The probabilistic model is zero-inflated:
        y = active * magnitude
    A fully unconditional forecast roughly behaves like:
        occ_prob * conditional_magnitude_quantile

    This can be too conservative for WAPE/penalty when occ_prob is high but not 1.
    Hybrid output uses a softened occurrence discount:
        occ_weight = min(1, occ_prob / tau)
        forecast = occ_weight * conditional_magnitude_quantile

    If occ_prob >= tau, the forecast uses the conditional magnitude quantile directly.
    If occ_prob < tau, the forecast is discounted.
    """
    model.eval()
    ...
    with torch.no_grad():
        ...
        for _ in range(M):
            ...
            occ_weight50 = torch.clamp(occ_prob / tau50, max=1.0)
            occ_weight70 = torch.clamp(occ_prob / tau70, max=1.0)

            p50_z = occ_weight50 * mag_p50
            p70_z = occ_weight70 * mag_p70
...
def generate_forecast_df_hybrid(
    model,
    va_ld,
    M=200,
    tau50=0.60,
    ...
    """
    Generate forecast_df using the hybrid output rule.
    ...
    with torch.no_grad():
        for b in va_ld:
            ...
            p50, p70, occ_mean, loc_mean, scale_mean = predict_hybrid_quantile(
                model,
                x,
                fc,
                M=M,
            ...
            for i in range(y.shape[0]):
                for h in range(y.shape[1]):
                    rows.append({
                        ...
                        "p50_scot": hm50[i, h].item(),
                        "p70_scot": hm70[i, h].item(),
                        "occ_prob_mean": occ_mean[i, h].item(),
                        "loc_mean": loc_mean[i, h].item(),
                        "scale_mean": scale_mean[i, h].item(),
                    })

    return pd.DataFrame(rows)


def _run_wape_for_forecast_df(forecast_df, label="forecast"):
    """
    Run your existing WAPE/penalty functions on a forecast_df.
    Requires:
      calculate_wape_using_lp_oos2
      quick_error_check
    """
    ...
    print("\n" + "=" * 80)
    print(f"WAPE CHECK: {label}")
    print("=" * 80)
    print("forecast rows:", len(forecast_df))
    print("forecast ASINs:", forecast_df["asin"].nunique())
    print("forecast zero rate:", (forecast_df["fbi_demand"] == 0).mean())
    print("forecast demand sum:", forecast_df["fbi_demand"].sum())
    print("p50 mean:", forecast_df["p50_amxl"].mean())
    print("p70 mean:", forecast_df["p70_amxl"].mean())
...
def run_output_rule_comparison(
    result,
    M=200,
    tau_grid=(0.50, 0.60, 0.70, 0.80),
):
    """
    Compare three output rules on the SAME trained model:

    A. Original sampling forecast:
       result["forecast_df"]

    B. Smooth forecast:
       occ_prob * conditional magnitude quantile

    C. Hybrid forecast:
       min(1, occ_prob / tau) * conditional magnitude quantile
    ...
    # A. Original sampling forecast already generated during training.
    if "forecast_df" in result and result["forecast_df"] is not None:
        all_outputs["original_sampling"] = _run_wape_for_forecast_df(
            result["forecast_df"],
            label="A. original zero-inflated sampling"
        )
    ...
    all_outputs["smooth_occ_times_mag"] = _run_wape_for_forecast_df(
        forecast_df_smooth,
        label="B. smooth occ_prob × conditional magnitude"
    )

    # C. Hybrid forecast over tau grid.
    hybrid_summary_rows = []

    for tau in tau_grid:
        forecast_df_hybrid = generate_forecast_df_hybrid(
            model,
            va_ld,
            M=M,
            tau50=tau,
        ...
        label = f"C. hybrid tau={tau:.2f}"
        out = _run_wape_for_forecast_df(
            forecast_df_hybrid,
            label=label
        )

        key = f"hybrid_tau_{tau:.2f}"
        all_outputs[key] = out

        row = {
            "rule": key,
            "tau": tau,
            "forecast_rows": len(forecast_df_hybrid),
            "forecast_asins": forecast_df_hybrid["asin"].nunique(),
            "p50_mean": forecast_df_hybrid["p50_amxl"].mean(),
            "p70_mean": forecast_df_hybrid["p70_amxl"].mean(),
            "true_mean": forecast_df_hybrid["fbi_demand"].mean(),
            "true_active_ratio": (forecast_df_hybrid["fbi_demand"] > 0).mean(),
            "p50_active_ratio": (forecast_df_hybrid["p50_amxl"] > 0.5).mean(),
            "p70_active_ratio": (forecast_df_hybrid["p70_amxl"] > 0.5).mean(),
        }

        if "p50_penalty_diff" in out:
            row["p50_penalty_diff"] = out["p50_penalty_diff"]
        if "p70_penalty_diff" in out:
            row["p70_penalty_diff"] = out["p70_penalty_diff"]
    ...
    print("\n" + "=" * 80)
    print("HYBRID OUTPUT RULE SUMMARY")
    print("=" * 80)
    print(hybrid_summary)

    all_outputs["hybrid_summary"] = hybrid_summary

    return all_outputs


def hybrid_active_magnitude_diagnosis(
    result,
    tau=0.70,
    M=200,
):
    """
    Diagnosis for hybrid output on active validation rows.
    """
    model = result["model"]
    va_ld = result["va_ld"]
    ...
    with torch.no_grad():
        for b in va_ld:
            ...
            cond_p50 = torch.expm1(loc_mean).clamp(min=0)
            cond_p70 = torch.exp(loc_mean + 0.5244 * scale_mean).sub(1.0).clamp(min=0)
            ...
            if pos.sum() > 0:
                rows.append({
                    "true_active_mean": y[pos].mean().item(),
                    "cond_p50_active_mean": cond_p50[pos].mean().item(),
                    "cond_p70_active_mean": cond_p70[pos].mean().item(),
                    "hybrid_p50_active_mean": p50[pos].mean().item(),
                    "hybrid_p70_active_mean": p70[pos].mean().item(),
                    "occ_active_mean": occ_mean[pos].mean().item(),
                    "occ_active_p25": occ_mean[pos].quantile(0.25).item(),
                    "occ_active_p50": occ_mean[pos].quantile(0.50).item(),
                    "occ_active_p75": occ_mean[pos].quantile(0.75).item(),
                })

    diag = pd.DataFrame(rows)

    print("\n" + "=" * 80)
    print(f"HYBRID ACTIVE MAGNITUDE DIAGNOSIS tau={tau}")
    print("=" * 80)
    ...
    print(diag.describe().T)

    ratio_df = pd.DataFrame([{
        "cond_p50 / true": diag["cond_p50_active_mean"].mean() / diag["true_active_mean"].mean(),
        "cond_p70 / true": diag["cond_p70_active_mean"].mean() / diag["true_active_mean"].mean(),
        "hybrid_p50 / true": diag["hybrid_p50_active_mean"].mean() / diag["true_active_mean"].mean(),
        "hybrid_p70 / true": diag["hybrid_p70_active_mean"].mean() / diag["true_active_mean"].mean(),
    }]).T.rename(columns={0: "ratio"})
