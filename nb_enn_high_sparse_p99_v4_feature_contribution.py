...
def load_real_data(data_raw):
    ...
    context_cols = ["our_price"] + dph_cols + promo_context_cols + holiday_cols
    ...
    cat_codes, cat_uniques = pd.factorize(df["category_code"])
    df["category_id"] = cat_codes.astype(int) + 1
    ...
    for asin, group in df.groupby("ASIN"):
        ...
        category_id = int(_mode_or_zero(group["category_id"]))
...
def main(
    ...
    print("NB-v4 | 28-feature TCN + Category Embedding + ENN | safe in_stock_dph + promotion_ratio | p99 clean high-sparse | contribution diagnostics")
    print("=" * 60)
...
# =====================================================
# 14. Feature Contribution / Masking Diagnostics
# =====================================================

def _get_wape_penalty_values(forecast_df):
    """
    Run WAPE and extract main AMXL penalty / overbias / underbias values.
    Requires your notebook functions:
      calculate_wape_using_lp_oos2
      quick_error_check
    """
    if (
        "calculate_wape_using_lp_oos2" not in globals()
        or "quick_error_check" not in globals()
    ):
        raise RuntimeError(
            "calculate_wape_using_lp_oos2 or quick_error_check not found."
        )
    ...
    def get_scalar(obj, key):
        if isinstance(obj, pd.Series):
            return float(obj[key])
        if isinstance(obj, pd.DataFrame):
            if key in obj.index:
                return float(obj.loc[key].values[0])
            if key in obj.columns:
                return float(obj[key].values[0])
        if isinstance(obj, dict):
            return float(obj[key])
        return np.nan

    out = {
        "p50_amxl_penalty": get_scalar(p50_wape, "p50_amxl_penalty"),
        "p50_amxl_overbias": get_scalar(p50_wape, "p50_amxl_overbias"),
        "p50_amxl_underbias": get_scalar(p50_wape, "p50_amxl_underbias"),
        "p70_amxl_penalty": get_scalar(p70_wape, "p70_amxl_penalty"),
        "p70_amxl_overbias": get_scalar(p70_wape, "p70_amxl_overbias"),
        "p70_amxl_underbias": get_scalar(p70_wape, "p70_amxl_underbias"),
...
def make_masked_batches(
    va_ld,
    history_feature_indices=None,
    future_context_indices=None,
    mask_category=False,
):
    """
    Make copied validation batches with selected groups masked.

    history_feature_indices:
      indices in x[:, :, feature_idx] to zero out.

    future_context_indices:
      indices in future_context[:, :, context_idx] to zero out.

    mask_category:
      if True, category_id is set to 0.
    """
    masked_batches = []

    for b in va_ld:
        nb = _copy_batch_dict(b)

        if history_feature_indices is not None and len(history_feature_indices) > 0:
            nb["x"][:, :, history_feature_indices] = 0.0

        if future_context_indices is not None and len(future_context_indices) > 0:
            nb["future_context"][:, :, future_context_indices] = 0.0

        if mask_category and "category_id" in nb:
            nb["category_id"] = torch.zeros_like(nb["category_id"])

        masked_batches.append(nb)

    return masked_batches


def generate_forecast_df_from_batches(model, batches, M=100):
    """
    Generate forecast_df from already-created batches.
    Used for feature masking contribution.
    """
    rows = []
    model.eval()
...
def default_nb_v4_history_feature_groups():
    """
    Feature groups for NB-v4 promotion_ratio-only version.

    History encoder feature indices:
      0  log1p(demand)
      1  active indicator
      2  distance since last active
      3  yearly sin
      4  yearly cos
      5  promo_t
      6  quarterly sin
      7  quarterly cos
      8  hist_nonzero_mean_52_log
      9  hist_nonzero_p75_52_log
      10 recent_peak_13_log
      ...
      20 price_log
      21 positive_mean_4_log
      22 positive_mean_13_log
      23 positive_max_13_log
      24 positive_std_13_log
      25 promotion_ratio
      26 promotion_ratio_mean_4
      27 promotion_ratio_mean_13
    """
    return {
        "base_demand": [0, 1, 2],
        "seasonality": [3, 4, 6, 7],
        "holiday_promo_window": [5],
        "magnitude_anchor": [8, 9, 10, 21, 22, 23, 24],
        "inventory_oos": [11, 12, 15, 16, 17, 18],
        "occurrence_rate": [13, 14, 19],
        "price_history": [20],
        "promotion_history": [25, 26, 27],
    }


def infer_future_context_groups_from_batch(va_ld):
    """
    Infer future context groups from the first batch if context column names
    are not directly stored in the batch.

    Since DataLoader batches do not carry context_cols, this assumes the
    current NB-v4 context order:
      ["our_price", "in_stock_dph", optional "promotion_ratio", holiday_indicator_*]
    If promotion_ratio is absent, the context_dim may be 2 + holidays.
    """
    b0 = next(iter(va_ld))
    C = b0["future_context"].shape[-1]

    groups = {}

    if C >= 1:
        groups["future_price"] = [0]

    if C >= 2:
        groups["future_instock_safe"] = [1]

    if C >= 3:
        # In NB-v4 promotion-ratio-only, promotion_ratio is inserted before holidays.
        groups["future_promotion_ratio"] = [2]

    if C > 3:
        groups["future_holiday_indicators"] = list(range(3, C))

    return groups


def evaluate_feature_group_contribution(
    model,
    va_ld,
    base_forecast_df,
    history_groups=None,
    future_context_groups=None,
    include_category=True,
    M=100,
):
    """
    Fast contribution test by validation masking.

    Interpretation:
      delta_p70_penalty > 0:
          masking this group makes penalty worse,
          so this group is useful / positive contribution.

      delta_p70_penalty < 0:
          masking this group makes penalty better,
          so this group may be noisy / negative contribution.

    Important:
      This is a fast diagnostic, not a final causal conclusion.
      If a group looks negative, retrain without it to confirm.
    """
    if history_groups is None:
        history_groups = default_nb_v4_history_feature_groups()

    if future_context_groups is None:
        future_context_groups = infer_future_context_groups_from_batch(va_ld)

    results = []

    print("\n" + "=" * 80)
    print("FEATURE CONTRIBUTION BY MASKING")
    print("=" * 80)

    base_vals, base_wape_df = _get_wape_penalty_values(base_forecast_df)

    base_row = {
        "group": "BASE",
        "type": "base",
        **base_vals,
        "delta_p50_penalty": 0.0,
        "delta_p70_penalty": 0.0,
        "delta_p50_underbias": 0.0,
        "delta_p70_underbias": 0.0,
        "delta_p50_overbias": 0.0,
        "delta_p70_overbias": 0.0,
    }
    results.append(base_row)

    # History groups
    for name, idxs in history_groups.items():
        print(f"\nMasking history group: {name} | idx={idxs}")

        batches = make_masked_batches(
            va_ld,
            history_feature_indices=idxs,
            future_context_indices=None,
            mask_category=False,
        )

        fcst = generate_forecast_df_from_batches(
            model,
            batches,
            M=M
        )

        vals, _ = _get_wape_penalty_values(fcst)

        row = {
            "group": name,
            "type": "history",
            **vals,
            "delta_p50_penalty": vals["p50_amxl_penalty"] - base_vals["p50_amxl_penalty"],
            "delta_p70_penalty": vals["p70_amxl_penalty"] - base_vals["p70_amxl_penalty"],
            "delta_p50_underbias": vals["p50_amxl_underbias"] - base_vals["p50_amxl_underbias"],
            "delta_p70_underbias": vals["p70_amxl_underbias"] - base_vals["p70_amxl_underbias"],
            "delta_p50_overbias": vals["p50_amxl_overbias"] - base_vals["p50_amxl_overbias"],
            "delta_p70_overbias": vals["p70_amxl_overbias"] - base_vals["p70_amxl_overbias"],
        }

        results.append(row)

    # Future context groups
    for name, idxs in future_context_groups.items():
        print(f"\nMasking future context group: {name} | idx={idxs}")

        batches = make_masked_batches(
            va_ld,
            history_feature_indices=None,
            future_context_indices=idxs,
            mask_category=False,
        )
    ...
    print("\nSorted by delta_p70_penalty descending:")
    print(out[cols].sort_values("delta_p70_penalty", ascending=False))

    print("\nInterpretation:")
    print("delta_p70_penalty > 0: masking hurts, feature group is useful.")
    print("delta_p70_penalty < 0: masking helps, feature group may be noisy/negative.")
    print("For final conclusion, retrain without suspicious groups.")

    return out


def run_feature_contribution_check(
    model,
    va_ld,
    forecast_df,
    M=100,
    ...
    """
    One-call feature contribution check using default NB-v4 feature groups.
    """
    history_groups = default_nb_v4_history_feature_groups()
    future_context_groups = infer_future_context_groups_from_batch(va_ld)

    print("\nDefault history feature groups:")
    for k, v in history_groups.items():
        print(k, ":", v)

    print("\nInferred future context groups:")
    for k, v in future_context_groups.items():
        print(k, ":", v)

    contrib_df = evaluate_feature_group_contribution(
        model=model,
        va_ld=va_ld,
        base_forecast_df=forecast_df,
        history_groups=history_groups,
        future_context_groups=future_context_groups,
        include_category=True,
        M=M,
    )
